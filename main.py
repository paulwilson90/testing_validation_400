...
def all_data(all_row_data):
    ...
    ICE_ON_not_company_corrected = ice_protect_addit(flap, corrected_for_prop) / percent_increase
...
def write_to_excel(all_exc):
    ...
    writer = pd.ExcelWriter('400_tests_run.xlsx')
    ...
    df = df.style.applymap(lambda x: 'background-color: red' if '*' in str(x) else ('background-color: orange' if
                                                                                    '^' in str(x) else ''))
    df.to_excel(writer, sheet_name='Q400 system errors', index=False)
